...
拆分器在识别边界、标题与摘要前屏蔽完整 HTML 注释（包括跨段落注释），但保留换行和字符位置；原文片段与哈希仍来自未经改写的页面。注释中的模板说明、标题、分隔线和关键词不参与新闻或实体抽取。新闻标题必须包含可检索文字，不能只有标点或 HTML 残片。
...
验证器不仅比较已生成 JSON，还重新读取每个上游页面和每条新闻片段：
页面 SHA-256、片段行列范围与片段 SHA-256 任一漂移都会失败，避免
processed news 与原文在结构校验仍通过时静默分叉。

## 检索投影

站点为每条事件构建确定性的检索文档，包含标题、摘要、具体事件类型、
实体名称与别名、实体类型、来源栏目和日期。查询与文档使用同一套
Unicode 兼容归一化，忽略大小写、全半角、空白和标点差异，并扩展
经审查的缩写、同义词及行政区后缀。多个查询词保持 AND 语义，
同义表达在单个词内使用 OR，兼顾召回率与可解释性。
